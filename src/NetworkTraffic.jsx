...
const NetworkFlowVisualizer = () => {
  ...
  useEffect(() => {
    ...
    // Elementos de la red
    const elements = {
      user1: { x: width * 0.1, y: height * 0.4, size: 40, label: 'Usuario 1', image: '🧑‍💻' },
      user2: { x: width * 0.1, y: height * 0.6, size: 40, label: 'Usuario 2', image: '👩‍💻' },
      router1: { x: width * 0.4, y: height / 2, size: 30, label: 'Router 1', image: '📡' },
      router2: { x: width * 0.6, y: height / 2, size: 30, label: 'Router 2', image: '📡' },
      server: { x: width * 0.85, y: height / 2, size: 40, label: 'Servidor Web', image: '🌐' },
    };

    // Líneas de conexión
    const lines = [
      { id: 'u1-r1', start: elements.user1, end: elements.router1, color: '#63b3ed' },
      { id: 'u2-r1', start: elements.user2, end: elements.router1, color: '#63b3ed' },
      { id: 'r1-r2', start: elements.router1, end: elements.router2, color: '#63b3ed' },
      { id: 'r2-s', start: elements.router2, end: elements.server, color: '#63b3ed' },
      { id: 's-r2', start: elements.server, end: elements.router2, color: '#63b3ed' },
      { id: 'r2-r1', start: elements.router2, end: elements.router1, color: '#63b3ed' },
      { id: 'r1-u1', start: elements.router1, end: elements.user1, color: '#63b3ed' },
      { id: 'u1-u2', start: elements.user1, end: elements.user2, color: '#63b3ed' },
    ];
    ...
    const getPath = (pathIds) => pathIds.map(id => lines.find(l => l.id === id));
    ...
    const animate = (currentTime) => {
      ...
      switch (animationDataRef.current.animationState) {
        case 'idle':
          if (now - animationDataRef.current.lastStateChangeTime > stateWaitTime) {
          ...
          break;
        case 'request':
          getPath(requestPath).forEach(line => drawLine(line.start, line.end, activeLineColor));
          if (animationDataRef.current.packetsToSend > 0 && now - lastPacketSent > packetDelay) {
            sendPacket(elements.user1, elements.server, '#ffc658', requestPath);
            animationDataRef.current.packetsToSend--;
            lastPacketSent = now;
          }
          if (dataPackets.length === 0 && animationDataRef.current.packetsToSend === 0) {
          ...
          break;
        case 'response':
          getPath(responsePath).forEach(line => drawLine(line.start, line.end, activeLineColor));
          if (animationDataRef.current.packetsToSend > 0 && now - lastPacketSent > packetDelay) {
            sendPacket(elements.server, elements.user1, '#82ca9d', responsePath);
            animationDataRef.current.packetsToSend--;
            lastPacketSent = now;
          }
          if (dataPackets.length === 0 && animationDataRef.current.packetsToSend === 0) {
          ...
          break;
        case 'share':
          getPath(sharePath).forEach(line => drawLine(line.start, line.end, activeLineColor));
          if (animationDataRef.current.packetsToSend > 0 && now - lastPacketSent > packetDelay) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-gray-900 rounded-3xl shadow-2xl m-4">
      <div className="w-full flex justify-center">
        <canvas
          ref={canvasRef}
          className="bg-gray-800 rounded-2xl border border-gray-700 shadow-inner w-full h-auto"
